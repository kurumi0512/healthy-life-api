...
public class KeyUtil {
	 ...
	 */
	public static boolean verifySignature(PublicKey publicKey, String message, byte[] digitalSignature, String cypto)
			throws Exception {
	 ...
	 */
	public static boolean verifySignatureFromMessage(PublicKey publicKey, String message, byte[] digitalSignature)
			throws Exception {
	 ...
	 * @throws Exception 若簽名驗證過程中發生錯誤時拋出。
	 */
	public static boolean verifySignatureFromFile(PublicKey publicKey, String filePath, byte[] digitalSignature)
			throws Exception {
	...
	public static byte[] decryptWithPrivateKey(PrivateKey privateKey, byte[] encryptedData) throws Exception {
		...
		int chunkLength = (privateKey instanceof RSAPrivateKey)
				? ((RSAPrivateKey) privateKey).getModulus().bitLength() / 8
				: CHUNK_SIZE;
	 ...
	 * @throws Exception 若在解密過程中出現問題時拋出。
	 */
	public static String decryptWithAESKeyAndIVInCTRMode(SecretKeySpec key, byte[] encryptedData, byte[] iv)
			throws Exception {
		// 當使用AES的CTR模式時，通常使用"NoPadding"作為填充方案，而不是"PKCS5Padding"。
		// 這是因為CTR模式將密鑰轉化為一個字節流，該字節流與原始數據進行XOR操作，因此不需要任何填充。
		Cipher cipher = Cipher.getInstance("AES/CTR/NoPadding");
	...
	/**
	 * 可將字節陣列轉換為十六進制格式的字符串。 這通常用於方便地顯示二進制數據，如數字簽名、摘要或加密的數據。
	 * 
	 ...
	 * @return 返回文件內容的 SHA-256 雜湊值，以十六進制字符串格式表示。 如果在讀取文件或生成雜湊時出現錯誤，則返回 null。
	 */
	 ...
	 * 用於檢查訊息的完整性並提供身份驗證，特別是在密碼學中。 Hmac 需要一個加密哈希函數和一個密鑰作為其兩個主要參數。
	 * 
	 * SHA256 - 這是一個加密哈希函數， 屬於 SHA-2（Secure Hash Algorithm 2）家族。256 表示輸出的摘要/哈希長度是
	 * 256 位。
	 * 
	 * 除了 HmacSHA256 外，還有其他的 Hmac 設定，這些設定主要根據所使用的加密哈希函數而有所不同。以下是一些常見的選項： HmacSHA1 -
	 * 使用 SHA-1 加密哈希函數。 HmacSHA384 - 使用 SHA-2 家族中的 SHA-384 函數。 HmacSHA512 - 使用 SHA-2
	 * 家族中的 SHA-512 函數。
	 * 
	 ...
	 *
	 *         使用方法: - SecureRandom 用於生成加密安全的隨機數。 - 這個方法首先使用 SecureRandom
	 *         生成指定長度的隨機密鑰。 - 然後將生成的密鑰進行 Base64 編碼，這樣可以將它安全地存儲或轉輸，同時避免任何不打印或特殊的字符。
	 */
	...
	public static JWTClaimsSet createJWTClaims(String subject, String issuer, String name) {
		return new JWTClaimsSet.Builder().subject(subject).issuer(issuer).claim("name", name).build();
	}
	...
	public static String encryptJWT(SignedJWT signedJWT, String encryptionSecret) throws JOSEException {
		JWEObject jweObject = new JWEObject(
				new JWEHeader.Builder(JWEAlgorithm.DIR, EncryptionMethod.A192GCM).contentType("JWT").build(),
	 ...
	 */
	public static SignedJWT decryptJWT(String encryptedJWT, String decryptionSecret)
